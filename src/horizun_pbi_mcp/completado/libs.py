...
# esquemas. Esto llevaba su propia promocion -`_promover_directorio`,
# `_recuperar_interrupcion`, un `.previous` de nombre fijo- y funcionaba; el
# problema no era que estuviera mal, sino que era la TERCERA.
from horizun_pbi_mcp.lifecycle import locking as cerrojos
from horizun_pbi_mcp.lifecycle import promotion

#: El manifiesto viaja EN el paquete: es la lista de versiones y hashes, y sin
#: ella una instalacion por `pip` no puede completarse a si misma. Lo que no
#: viaja son las DLL, que son binarios de Microsoft.
MANIFIESTO = Path(__file__).with_name("libs_manifest.json")

#: Donde lee el servidor. Antes esto apuntaba a `<checkout>/libs`, que es el
#: sitio correcto para el clon y **ninguno** para alguien que instalo por `pip`.
#: Se resuelve al importar y se puede sustituir -`--dest`, o las pruebas-.
LIBS = None
...
def destino() -> Path:
    """Donde se instalan las DLL: lo que se pidio, o donde lee el servidor."""
    if LIBS is not None:
        return Path(LIBS)
    from horizun_pbi_mcp.config import get_settings

    return get_settings().libs_dir
...
def estado() -> Dict[str, Any]:
    ...
    try:
        m = leer_manifiesto()
        return _estado_directorio(destino(), m)
...
def instalar() -> Dict[str, Any]:
    ...
    with cerrojos.CerrojoDeCicloDeVida(raiz, etiqueta="libs") as cerrojo:
        ...
        staging = promotion.crear_staging(raiz, destino_dll.name)
        try:
            ...
            promotion.promover(raiz, staging, destino_dll)
        ...
        recogidos = promotion.limpiar_apartados_de(raiz, destino_dll.name)

    final = estado()
    if not final["ready"]:
        raise LibsError(f"Tras instalar, la verificacion falla: {final['reason']}")
    return {"installed": len(m["files"]), "dir": str(destino_dll),
            "version": VERSION_FIJADA,
